...
cur.execute("SELECT record FROM records")
if cur.fetchone() is None:
    cur.execute(f"INSERT INTO records VALUES (?)", "0")
    db.commit()
# ----------------------------------------------------------------------
...
class Game(Ursina):
    def __init__(self):
        ...
        Light(type="directional", color=(0.5, 0.5, 0.5, 1), directional=(1, 1, 1))
        self.MAP_SIZE = 20
        self.new_game()
    ...
    def check_game_over(self):
        ...
        if 0 < snake[-1][0] < self.MAP_SIZE and 0 < snake[-1][1] < self.MAP_SIZE and len(snake) == len(set(snake)):
            self.set_record(self.record, self.snake.score)
            return
    ...
    def get_record(self):
        for value in cur.execute("SELECT record FROM records"):
            return value[0]

    def set_record(self, record, score):
        rec = max(int(record), score)
        cur.execute(f"UPDATE records SET record = {str(rec)}")
        db.commit()

    def update(self):
        """ Updates for every frame """
        self.record = self.get_record()
        print_on_screen(f'Score:     {self.snake.score}\nRecord:  {self.record}', position=(-0.85, 0.45), scale=3, duration=1 / 20)
